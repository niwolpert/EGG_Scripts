...
This code is provided without explicit or implied guarantee, and without any 
...
2. All the functions are called from 'Script_EGG_main'. Exemple data sets are
provided. Download the data sets and note the path and name in Script_EGG_main,
as well as the path to the scripts and the fieldtrip toolbox

3. Call 'compute_FFT_EGG'
Input: Filename (as string) and channel labels (cell of strings)
This function computes a power spectrum for all the specified channels and
outputs a fieldtrip structure contianting the power spectrum.
...
Input: Filename (as string), power spectrum (fieldtrip structure output
from 'compute_FFT_EGG'), downsampling frequency and channel labels
This function computes the filter, amplitude and phase of the EGG of the
channel with the highest peak in the power spectrum in the normogastric range.
It outputs a fieldtrip structure with the filtered signal, amplitude and phase.

5. Call 'compute_filter_EGG'
...
Input: Filtered signal (output from 'compute_filter_EGG')
This function creates a plot with a histogram of cycle duration, with dotted
red lines marking the mean +/- 3 standard deviations.

7. Call 'detect_EGG_artifacts'
Input: Filtered signal (output from 'compute_filter_EGG')
This function looks for artifact cycles in the phase time series. The two 
...
(fourth row). Artifact cycles are shaded in red in the third row.